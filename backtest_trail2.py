"""
Sweep trailing stop % for SOLEUR on 15m with current settings
(sell>75, TP=5%, floor=3%, RSI14 buy<30).

Usage:
    python backtest_trail2.py [days]   # default: 365 180
"""
import sys
...
def run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr,
        start_balance: float, trail_pct: float) -> tuple[list[Trade], float]:
...
def report(label: str, trades: list[Trade]):
    ...
    if not realized:
        print(f"  {label:<30}  no closed trades")
        return
    ...
    print(f"  {label:<30}  n={n:>3}{eod}  W/L={wins}/{n-wins}"
...
if __name__ == "__main__":
    ...
    print(f"\nTrailing stop sweep — SOLEUR 15m RSI({config.rsi_period_for('SOLEUR')})")
    print(f"floor={config.PROFIT_FLOOR_PCT*100:.0f}%  sell>{config.RSI_OVERBOUGHT}"
          f"  TP={config.TAKE_PROFIT_PCT*100:.0f}%  pos={config.POSITION_SIZE_PCT*100:.0f}%"
          f"  buy<{config.RSI_OVERSOLD}\n")

    for days in days_list:
        print(f"{'━'*105}")
        print(f"  {days}d window")
        print(f"{'━'*105}")
        print(f"  {'Scenario':<30}  {'n':>4}       {'W/L':<7}  {'PnL':>8}  {'avg':>6}"
              f"  {'worst':>7}  {'best':>6}  fees   exits")
        print(f"  {'─'*30}  {'─'*4}  {'─'*5}  {'─'*7}  {'─'*8}  {'─'*6}"
              f"  {'─'*7}  {'─'*6}  {'─'*5}  {'─'*22}")

        df = fetch(pair, days)
        rsi_arr, ema_arr, close_arr, hi_arr, lo_arr = precompute(df)

        for trail_pct, label in scenarios:
            trades, _ = run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr, start, trail_pct)
            report(label, trades)
        print()

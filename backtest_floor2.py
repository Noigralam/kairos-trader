"""
Sweep profit floor % for SOLEUR on 15m with current settings
(sell>75, TP=5%, trail=5%, RSI14 buy<30).

Usage:
    python backtest_floor2.py [days]   # default: 365 180
"""
import sys
...
def run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr,
        start_balance: float, floor_pct: float) -> tuple[list[Trade], float]:
...
if __name__ == "__main__":
    ...
    print(f"\nProfit floor sweep — SOLEUR 15m RSI({config.rsi_period_for('SOLEUR')})")
    print(f"trail={config.TRAILING_STOP_PCT*100:.0f}%  sell>{config.RSI_OVERBOUGHT}"
    ...
    for days in days_list:
        ...
        for floor_pct, label in scenarios:
            trades, _ = run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr, start, floor_pct)
            report(label, trades)
        print()

"""
Backtest trailing stop % sweep for SOLEUR.

Usage:
    python backtest_trail.py [days]   # default: 365
"""
import sys
...
def run(df: pd.DataFrame, start_balance: float,
        trail_pct: float, tp_pct: float, profit_floor: float) -> tuple[list[Trade], float]:
...
def report_line(label: str, trades: list[Trade], start: float, end: float):
    ...
    print(f"  {label:<32}  n={len(trades):>3}  W/L={wins}/{len(trades)-wins}"
          f"  PnL={pnl:>+7.2f}  after-tax={after:>7.2f}"
          f"  worst={worst:>+6.2f}  fees={fees:.2f}"
          f"  TP×{tp_count} stop×{stop_count} sig×{sig_count}")


if __name__ == "__main__":
    days = int(sys.argv[1]) if len(sys.argv) > 1 and sys.argv[1].isdigit() else 365
    print(f"\nTrailing stop sweep — {PAIR} — {days}d")
    print(f"pos={config.POSITION_SIZE_PCT*100:.0f}%  dca={config.DCA_SIZE_PCT*100:.0f}%"
          f"  drop={config.DCA_DROP_PCT*100:.0f}%  min_exit={config.MIN_EXIT_PROFIT_PCT*100:.0f}%\n")

    df = fetch(days)
    start = config.SIMULATION_BALANCE

    # (label, trail_pct, tp_pct, profit_floor)
    scenarios = []
    for trail in [0.03, 0.04, 0.05, 0.06, 0.07, 0.08, 0.10]:
        for tp in [0.07, 0.10, 0.15]:
            scenarios.append((f"trail={trail*100:.0f}%  TP={tp*100:.0f}%", trail, tp, 0.01))

    print(f"  {'Scenario':<32}  {'n':>4}  {'W/L':<7}  {'PnL':>8}  {'after-tax':>10}  {'worst':>7}  {'fees':>5}  exits")
    print(f"  {'─'*32}  {'─'*4}  {'─'*7}  {'─'*8}  {'─'*10}  {'─'*7}  {'─'*5}  {'─'*20}")

    for label, trail, tp, floor in scenarios:
        trades, end = run(df, start, trail, tp, floor)

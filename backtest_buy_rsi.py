"""
Sweep buy RSI threshold for SOLEUR on 15m with current settings
(sell>75, TP=5%, trail=5%, floor=3%, RSI14).

Usage:
    python backtest_buy_rsi.py [days]   # default: 365 180
"""
import sys
...
def run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr,
        start_balance: float, rsi_buy: int) -> tuple[list[Trade], float]:
...
def report(label: str, trades: list[Trade]):
    ...
    if not realized:
        print(f"  {label:<32}  no closed trades")
        return
    ...
    print(f"  {label:<32}  n={n:>3}{eod}  W/L={wins}/{losses}"
...
if __name__ == "__main__":
    ...
    print(f"\nBuy RSI threshold sweep — SOLEUR 15m RSI({config.rsi_period_for('SOLEUR')})")
    print(f"floor={config.PROFIT_FLOOR_PCT*100:.0f}%  trail={config.TRAILING_STOP_PCT*100:.0f}%"
          f"  sell>{config.RSI_OVERBOUGHT}  TP={config.TAKE_PROFIT_PCT*100:.0f}%"
          f"  pos={config.POSITION_SIZE_PCT*100:.0f}%\n")

    for days in days_list:
        print(f"{'━'*108}")
        print(f"  {days}d window")
        print(f"{'━'*108}")
        print(f"  {'Scenario':<32}  {'n':>4}       {'W/L':<7}  {'PnL':>8}  {'avg':>6}"
              f"  {'worst':>7}  {'best':>6}  fees   exits")
        print(f"  {'─'*32}  {'─'*4}  {'─'*5}  {'─'*7}  {'─'*8}  {'─'*6}"
              f"  {'─'*7}  {'─'*6}  {'─'*5}  {'─'*22}")

        df = fetch(pair, days)
        rsi_arr, ema_arr, close_arr, hi_arr, lo_arr = precompute(df)

        for rsi_buy, label in scenarios:
            trades, _ = run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr, start, rsi_buy)
            report(label, trades)
        print()

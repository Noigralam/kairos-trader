"""
Sweep take-profit % and sell RSI threshold for SOLEUR on 15m.
All other settings held at current values.

Usage:
    python backtest_exit.py [days]   # default: 365 180
"""
import sys
...
def run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr,
        start_balance: float, rsi_sell: int, tp_pct: float) -> tuple[list[Trade], float]:
...
if __name__ == "__main__":
    ...
    # (rsi_sell, tp_pct, label)
    scenarios = [
        (65, 0.10, "sell>65  TP=10%  ◄ current"),
        (65, 0.05, "sell>65  TP=5%"),
        (65, 0.07, "sell>65  TP=7%"),
        (70, 0.10, "sell>70  TP=10%"),
        (70, 0.05, "sell>70  TP=5%"),
        (70, 0.07, "sell>70  TP=7%"),
        (75, 0.10, "sell>75  TP=10%"),
        (75, 0.05, "sell>75  TP=5%"),
        (75, 0.07, "sell>75  TP=7%"),
    ]

    print(f"\nExit threshold sweep — SOLEUR 15m RSI({config.rsi_period_for('SOLEUR')})")
    print(f"floor={config.PROFIT_FLOOR_PCT*100:.0f}%  trail={config.TRAILING_STOP_PCT*100:.0f}%"
          f"  pos={config.POSITION_SIZE_PCT*100:.0f}%  buy<{config.RSI_OVERSOLD}\n")

    for days in days_list:
        print(f"{'━'*115}")
        print(f"  {days}d window")
        print(f"{'━'*115}")
        print(f"  {'Scenario':<44}  {'n':>4}       {'W/L':<7}  {'PnL':>8}  {'avg':>6}"
              f"  {'worst':>7}  {'best':>6}  fees   exits")
        print(f"  {'─'*44}  {'─'*4}  {'─'*5}  {'─'*7}  {'─'*8}  {'─'*6}"
              f"  {'─'*7}  {'─'*6}  {'─'*5}  {'─'*22}")

        df = fetch(pair, days)
        rsi_arr, ema_arr, close_arr, hi_arr, lo_arr = precompute(df)

        for rsi_sell, tp_pct, label in scenarios:
            trades, end = run(rsi_arr, ema_arr, close_arr, hi_arr, lo_arr,
                              start, rsi_sell, tp_pct)
